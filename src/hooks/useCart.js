...
const useCart = () => {
  ...
  useEffect(() => {
    fetch(`https://enigmatic-lowlands-23232.herokuapp.com/cart/${uid}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.length) {
  ...
  function addToCart(course) {
    ...
    if (isHave) {
      ...
      fetch("https://enigmatic-lowlands-23232.herokuapp.com/course/add", {
        method: "post",
  ...
  function remove(id) {
    fetch(`https://enigmatic-lowlands-23232.herokuapp.com/delete/${id}`, {
      method: "delete",
    })
      .then((res) => res.json())
      .then((data) => {
